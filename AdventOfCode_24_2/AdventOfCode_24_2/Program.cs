...
namespace AdventOfCode_24
{
    ...
    internal class Program
    {
        ...
        public static void AddNewEntries(Queue<(int column, int row, int minute)> queue, (int column, int row, int minute) cState, Dictionary<int, Dictionary<(int, int, Direction), Blizzard>> blizzConfigs, (int column, int row) endPos, string[] field, HashSet<(int, int, int)> seen)
        {
            ...
            int distance = endPos.column - cState.column + endPos.row - cState.row;
        ...
        static void Main(string[] args)
        {
            ...
            (int column, int row, int minute) cState = (1, 0, 0);
            for (int round = 0; round < 3; round++)
            {
                queue.Enqueue(cState);
                if (round == 0 || round == 2)
                    finalState = (field[0].Length - 2, field.Length - 1, 0);
                else
                    finalState = (1, 0, 0);
                while (true)
                {
                    cState = queue.Dequeue();
            ...
            Console.WriteLine(cState.minute);
